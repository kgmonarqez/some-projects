...
void Mechanics::event_activator(){
    ...
    if(Current_cell.get_event()){ 
        bool Save_status = Current_cell.get_event()->action(*this);
        notification();
        if(Save_status){
...
void Mechanics::add(MechanicsObserver* Ref){
    Observers.push_back(Ref);
    notification();
}

void Mechanics::remove(MechanicsObserver* Ref){
    Observers.remove(Ref);
}

void Mechanics::notification(){
